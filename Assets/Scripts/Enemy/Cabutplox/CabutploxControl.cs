...
public class CabutploxControl : MonoBehaviour {
	...
	void OnTriggerEnter2D( Collider2D obj ) {
		if( obj.CompareTag("Punch")){
			if(!morto){
				...
				GetComponent<Rigidbody2D>().AddForce(player.transform.right * player.transform.localScale.x * pullHitForce);
			}
		}
	}
	...
	public void AtirarLanca(){
		...
		l.GetComponent<Rigidbody2D>().AddForce (atirar.transform.right * force);
