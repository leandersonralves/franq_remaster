...
public class SceneManager : MonoBehaviour {
	...
	void Start ()
	{
		GetComponent<UIImage>().pixelInset = new Rect(0f, 0f, Screen.width, Screen.height);
		c.a = 0f;
		GetComponent<UIImage>().color = c;
//		if(instance != null)
//		{
...
//		}
	...
	IEnumerator Fade (string levelName, bool isIn, bool reload = false)
	{
		...
		Color colorCurrent = GetComponent<UIImage>().color;
		if(!isIn)
			colorTarget = c;
		...
		while(lerp < 1f)
		{
			...
			GetComponent<UIImage>().color = Color.Lerp (colorCurrent, colorTarget, lerp);
			yield return null;
		}
