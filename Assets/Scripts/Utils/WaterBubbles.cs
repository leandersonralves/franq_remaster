...
public class WaterBubbles : MonoBehaviour {
	...
	IEnumerator Start () {
		...
		GetComponent<ParticleSystem>().GetComponent<Renderer>().sortingLayerID = 3;
	...
	void Update () {
		...
		if(Vector3.Distance(m_transform.position, player.position) < distanceMinimum)
			player.GetComponent<Rigidbody2D>().AddForce((player.position - m_transform.position).normalized * force);
	}
}
