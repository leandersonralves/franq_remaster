...
public class Skills : MonoBehaviour
{
	...
	void Update () {
		...
		if(m_animator.GetCurrentAnimatorStateInfo(0).length > 0)
			inDefense = m_animator.GetCurrentAnimatorClipInfo(0)[0].clip.name.Contains("defense");

		if(Input.GetKeyDown (Button.Dash) && !m_animator.GetCurrentAnimatorClipInfo(0)[0].clip.name.Contains("walk"))
	...
	{
		if(isLiquid || m_animator.GetCurrentAnimatorClipInfo(0)[0].clip.name.Contains("liquid"))
			yield break;
