...
public class BubbleHydro : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		if(Input.GetKey(Button.Down) || Input.GetKey(Button.Up)){
			...
			bubble.GetComponent<Rigidbody2D>().AddForce(Vector3.up * v * velocityVertical * Time.deltaTime, ForceMode2D.Force);
//			bubble.transform.position += Vector3.up * v * velocityVertical;
		}

		if(Input.GetKey(Button.Left) || Input.GetKey(Button.Right)){
			float h = Input.GetAxis("Horizontal");
			bubble.GetComponent<Rigidbody2D>().AddForce(Vector3.right * h * velocityHorizontal * Time.deltaTime, ForceMode2D.Force);
//			bubble.transform.position += Vector3.right * h * velocityHorizontal;
		}

		if(bubble.GetComponent<Rigidbody2D>().velocity.magnitude > maxVelocity)
		{
			Vector3 currentVelocity = bubble.GetComponent<Rigidbody2D>().velocity;
			bubble.GetComponent<Rigidbody2D>().velocity = Vector3.ClampMagnitude(currentVelocity, maxVelocity);
		}
